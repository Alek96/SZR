...
``STATIC_ROOT``
    ...
    ``[project_root]/tmp/static``. Please refer to the `official settings
...
``MEDIA_ROOT``
    ...
    ``[project_root]/tmp/media``. Please refer to the `official settings
...
``SECRET_FILE``
    ...
    This file's location is set up here. Default value is ``[project_root]/tmp/SECRET.key``.
