...
all:
	...
	@echo "  run/frontend_dev    Runs server with frontend development settings"
	...
	@echo "  setup/frontend_dev  Runs setup/dev and configures django user"
	@echo "  update/dev          Shortcut for setup and refresh"
	@echo "  test [test_opt=]    Runs tests"
...
ifeq ($(DJANGO_TEST_SETTINGS_FILE),development)
	...
	@echo "Using setting file '$(DJANGO_TEST_SETTINGS_FILE)'..."
	@echo ""
	@$(PYTHON_BIN)/coverage run $(PYTHON_BIN)/django-admin.py test $(DJANGO_TEST_POSTFIX) --debug-mode ${test_opt}
endif
